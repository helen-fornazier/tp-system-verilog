if { ![project_exists {add_ieee}] } {
     project_new {add_ieee}
} else {
     project_open {add_ieee}
}
set_project_settings -cmp {add_ieee}
if { ![project_settings_exist -sim add_ieee] } {
        set_project_settings -sim add_ieee
}
set_global_assignment -section_id {add_ieee} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {VQM_FILE} {add_ieee.vqm}
...
if { [file exists {add_ieee_altera.prop}] } {
  source add_ieee_altera.prop
}
set_global_assignment -name {FAMILY} {CYCLONE II}
set_global_assignment -section_id {add_ieee} -name {DEVICE} {EP2C35F672C6}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {add_ieee_pnr_constraints.sdc}
